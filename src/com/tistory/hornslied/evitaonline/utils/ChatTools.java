...
public class ChatTools {
	...
	public static String formatTitle(String title) {
		...
		String center = ChatColor.YELLOW + " " + ChatColor.BOLD + title + ChatColor.WHITE + " "
				+ ChatColor.STRIKETHROUGH;
		String out = ChatColor.WHITE + line.substring(0, Math.max(0, (pivot - center.length() / 2)));
	...
	public static String formatTitle(String title, ChatColor color) {
		...
		String line = ChatColor.STRIKETHROUGH + "+------------------------------------------------+";
		int pivot = line.length() / 2;
		String center = color + " " + ChatColor.BOLD + title + ChatColor.WHITE + " " + ChatColor.STRIKETHROUGH;
		String out = ChatColor.WHITE + line.substring(0, Math.max(0, (pivot - center.length() / 2)));
		out += center + line.substring(pivot + center.length() / 2);
		return out;
	}
	...
	public static void main(String[] args) {
		...
		String[] players = { "dude", "bowie", "blarg", "sonbitch", "songoku", "pacman", "link", "stacker", "hacker",
				"newb" };
